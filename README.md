# 안녕하세요, 박민수입니다 👋

저는 **AI 솔루션과 업무 시스템을 실제 운영 가능한 형태로 구현하는 개발자**입니다.  
실시간 STT 연동, 온디바이스 AI 적용, FAQ 챗봇 구축, 사내 문서·결재 시스템 디지털 전환 등 다양한 프로젝트를 수행하며, 단순한 기능 구현을 넘어 **실제 현업에서 사용할 수 있는 서비스와 운영 구조를 만드는 경험**을 쌓아왔습니다.

## What I Do
- 실시간 STT 연동 및 음성 AI 적용
- FAQ 챗봇 구축 및 KB 데이터 파이프라인 자동화
- 사내 업무 시스템 디지털 전환 및 운영 자동화
- AI 모델의 서비스 적용 및 온디바이스 추론 구조 검토
- 실서비스 운영 중 발생하는 이슈 분석 및 안정화

## Featured Projects

### 1. Zoom 기반 연구노트·결재 통합 관리 시스템
- 사내 연구노트 작성, 결재, 문서 관리 프로세스를 웹 기반 통합 시스템으로 전환
- **1인 개발**로 기획, 설계, 프론트엔드, 백엔드, DB, 배포, 운영, 유지보수 수행
- 문서번호 자동화, 디지털 서명, PDF 저장, 관리자 운영 기능 구현

### 2. Zoom Virtual Agent 기반 FAQ 챗봇 구축 및 KB 데이터 파이프라인 · Contact Center 연동
- 비정형 CS 자료를 Zoom Docs 기반 KB로 표준화하고 CSV 변환 파이프라인 구축
- Zoom Virtual Agent/AI Companion 기반 FAQ 챗봇 시나리오 설계 및 인텐트 데이터 정비
- 챗봇 대화 맥락을 Zoom Contact Center 상담사 화면에 연동해 재질의 감소 및 운영 효율 개선

### 3. AICC 소리샘 탐지 모델 개선 및 온디바이스 적용
- 음성사서함(소리샘) 탐지 정확도 개선 및 고객사 배포형 추론 구조 적용 프로젝트
- 데이터셋 정비, 전처리 개선, 모델 성능 개선 협업, 온디바이스 적용 및 테스트 참여
- 팀 기준 탐지 정확도 **14%p 향상(66.53% → 97.85%)**

### 4. 실시간 STT 연동 및 모니터링 시스템
- Python(Asyncio/gRPC) 기반 실시간 STT 연동 클라이언트 및 모니터링 대시보드 구축
- TX/RX 로그 시각화, 운영 검증 환경, WAV 기반 장애 분석 기능 구현

## Tech
Python · Node.js · React · MySQL · PyTorch · TensorFlow Lite · gRPC · WebSocket · Zoom Apps SDK · Zoom Contact Center · Zoom Virtual Agent

## Links
- Portfolio: https://eight-penguin-b64.notion.site/174d25d0d71d80e5b0eec88949085272
- GitHub: https://github.com/hangpark123
- Email: minsu1009@naver.com
